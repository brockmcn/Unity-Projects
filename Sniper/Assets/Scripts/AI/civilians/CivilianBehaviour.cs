...
public class CivilianBehaviour : MonoBehaviour
{
    ...
    private CivilianAI civAi;

    // Use this for initialization
    void Start()
    {
        scaredWaypoints = null;
        _navAgent = GetComponent<NavMeshAgent>();
        civAi = GetComponent<CivilianAI>();
    ...
    void SetNextDestination(bool newDestination)
    {
        ...
        else
        {
            ...
            int randomDestination = Random.Range(0, scaredWaypoints.Count);
            Transform nextWaypointTransform = null;

            int nextWaypoint = (CurrentIndex == randomDestination) ? Random.Range(0, scaredWaypoints.Count) : randomDestination;
            nextWaypointTransform = scaredWaypoints[nextWaypoint];
    ...
    public void SetWaypoints(Vector3 scaredPos)
    {
        scaredWaypoints = new List<Transform>();
        foreach(Transform waypoint in civAi.wayPoints)
        {
            if (Vector3.Distance(scaredPos, waypoint.position) >= civAi.distanceAwayNotScared)
            {
                scaredWaypoints.Add(waypoint);
            }
        }
    }
}
